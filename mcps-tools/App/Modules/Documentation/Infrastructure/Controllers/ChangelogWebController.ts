...
import { ChangelogView } from "App/Modules/Documentation/Infrastructure/Views/ChangelogView.ts";

export class ChangelogWebController {

    private environmentReaderRawRepository: EnvironmentReaderRawRepository = EnvironmentReaderRawRepository.getInstance();

    public static getInstance(): ChangelogWebController {
        return new ChangelogWebController();
    }

    public async invoke(): Promise<CustomResponse> {
        try {
            const htmlContent: string = await ChangelogView.getInstance().invoke(
